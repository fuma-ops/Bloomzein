...
const PricingPage = lazyRetry(() => import("./pages/Pricing").then((m) => ({ default: m.PricingPage })));
...
function AppContent() {
  ...
  // Redirect /app or /app/ to /app/today — keep the query string so a Paddle
  // return (/app?checkout=success) still reaches PlusReturn on /app/today.
  useEffect(() => {
    if (path === "/app" || path === "/app/") {
      window.history.replaceState({}, "", "/app/today" + window.location.search);
      setPath("/app/today");
    }
  }, [path]);
