...
/* ============================================================
   CYCLE TRACKER — "Meet your cycle" onboarding
   ...
   sparkles are SVG (never colored emoji).
============================================================ */

type TourStep = { key: string; selector: string; title: string; desc: string };
...
export function CycleOnboarding({ onDone }: { onDone: () => void }) {
  ...
  // Deterministic sparkle field for the welcome/finale (no Math.random → stable).
  ...
  // Rising petals/hearts for depth.
  ...
  // Steps: find the real element → scroll to it → spotlight + auto-timer.
  useEffect(() => {
  ...
  // Self-drawing wheel geometry for the welcome act.
    ...
    <>
      ...
      {phase === "welcome" && (
        <div className="fixed inset-0 z-[10000] flex flex-col items-center justify-center overflow-hidden select-none"
          ...
          {/* soft glossy orb — a gentle halo, not a white-out */}
          ...
          <div className={`relative z-10 flex flex-col items-center text-center px-8 max-w-sm transition-all duration-[900ms] ease-out ${fadeIn ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
            <div className="text-[9px] font-black tracking-[0.42em] uppercase mb-5" style={{ color: "#DB2777", opacity: 0.55 }}>✦ your cycle ✦</div>

            {/* shimmering script title */}
            ...
            <div className="relative my-4" style={{ width: 120, height: 120, animation: "cyWheelFloat 4s ease-in-out infinite alternate", filter: "drop-shadow(0 10px 24px rgba(236,72,153,0.32))" }}>
              <svg width="120" height="120" viewBox="0 0 120 120">
                {/* glossy pearl backing so the pink ring pops on the bright wash */}
            ...
            <p className="text-[13px] leading-relaxed max-w-[250px] mb-8" style={{ color: "#9D5C7E", animation: "cyFadeUp 0.7s ease-out 0.5s both" }}>
              A soft little tour of your phases, your calendar &amp; your daily glow.
            </p>
            ...
            <button onClick={finish} className="mt-3 text-[11px] transition-colors" style={{ color: "#C48AAE" }}>
              skip
            </button>
          </div>
        </div>
      ...
      {phase === "steps" && (
        <>
          ...
          {/* light frosted spotlight — box-shadow paints a blush veil, hole stays crisp */}
          {spotRect && (
            <>
              ...
              {/* orbiting sparkles on the ring corners */}
        ...
        <div className="fixed inset-0 z-[10000] flex items-center justify-center overflow-hidden"
          ...
          <div className="text-center px-8" style={{ animation: "cyFadeUp 0.6s ease-out both" }}>
            ...
            <h2 className="font-script leading-none" style={{ fontSize: "clamp(2.5rem,11vw,3.75rem)", color: "#DB2777", filter: "drop-shadow(0 4px 16px rgba(236,72,153,0.3))" }}>
              You&apos;re in sync,<br />gorgeous
            </h2>
            <p className="mt-2 text-[13px]" style={{ color: "#9D5C7E" }}>Your bloom is ready. ✿</p>
          </div>
        </div>
      )}

      {/* keyframes — self-contained, pink-only, sparkle motion */}
      <style>{`
