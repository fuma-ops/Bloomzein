...
/** Category icon — the lucide Icon in palette pink, never a colored emoji. */
...
export function BudgetPlanner() {
  ...
  return (
    <div data-bp>
      {guideVisible && (
        <SparkleOnboarding
          content={BUDGET_GUIDE_CONTENT}
          steps={GUIDE_STEPS.map((s) => ({
            key: s.key, selector: s.selector, title: s.title, desc: s.desc,
            onEnter: () => setTab(s.tab),
          }))}
              ...
              return (
                <div key={key} className="shrink-0 flex flex-col items-center gap-1 w-16">
                  <div className={["grid h-11 w-11 place-items-center rounded-2xl text-xl shadow-sm border",
                    ...
                    <CatIcon cat={cat} className="h-5 w-5" color={isOver ? "#EF4444" : isUnplanned ? "#F59E0B" : "#EC4899"} />
                  </div>
