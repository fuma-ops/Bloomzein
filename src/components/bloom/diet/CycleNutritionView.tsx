...
export function CycleNutritionView({
  ...
  return (
    <div className="space-y-6">
      {/* ── Phase selector — tap any phase to explore it in place ── */}
      <div className="flex items-center gap-2 overflow-x-auto no-scrollbar -mx-1 px-1 pb-1">
        {CYCLE_PHASE_ORDER.map((p) => {
          ...
          return (
            <button
              ...
              className={[
                "group relative shrink-0 rounded-2xl border px-3.5 py-2 text-left transition active:scale-95",
                active
                  ? "border-hotpink bg-hotpink text-white shadow-md shadow-hotpink/30"
                  : "border-petal/60 bg-white/80 text-rose hover:bg-blush hover:-translate-y-0.5",
              ].join(" ")}
            >
              ...
              <span
                className={`block text-[10px] font-semibold ${active ? "text-white/80" : "text-rose/55"}`}
              >
                {PHASE_DAY_RANGE[p]}
              </span>
            </button>
        ...
        {!isToday && (
          <button
            ...
            className="shrink-0 inline-flex items-center gap-1 rounded-2xl border border-hotpink/50 bg-white/80 px-3 py-2 text-[12px] font-bold text-hotpink transition hover:bg-hotpink/10 active:scale-95 animate-selected-glow"
          >
            <CalendarHeart className="h-3.5 w-3.5" strokeWidth={2} /> Today
          </button>
        )}
      </div>

      {/* ── HERO · Your Body Today + At a Glance (right panel on desktop) ── */}
      <div key={selected} className="grid grid-cols-1 lg:grid-cols-5 gap-4 animate-fade-in">
        {/* Main — body today */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card className="relative overflow-hidden p-4 sm:p-5 bg-gradient-to-br from-blush/70 via-white to-petal/30">
          ...
          <p className="mt-2.5 text-[13px] leading-relaxed text-[#831843]">{content.didYouKnow}</p>
        </Card>
